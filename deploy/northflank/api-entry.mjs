...
export function northflankClientIp(headers) {
  ...
  if (!cloudflareNetworks.check(peer, family === 4 ? "ipv4" : "ipv6"))
    return peer;
  ...
  if (!client || !isIP(client))
    throw new Error("Invalid Cloudflare client address.");
  return client;
}
  ...
  return (request, ...arguments_) => {
    ...
    try {
      ...
      if (
        ...
        probes.has(new URL(request.url).pathname)
      )
...
export async function startNorthflankApi() {
  ...
  if (process.env.TRUSTED_CLIENT_IP_HEADER !== "cf-connecting-ip")
    throw new Error(
      "Northflank's adapter requires TRUSTED_CLIENT_IP_HEADER=cf-connecting-ip.",
    );
  ...
  const server = serve(
    { fetch: northflankFetch(runtime.app.fetch), port },
    (info) => {
      console.log(
        JSON.stringify({
          level: "info",

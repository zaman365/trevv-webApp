...
function operation(
  ...
  return {
    ...
    responses: {
      [status]: {
        description: "Scoped report, log or plan response",
...
export const reportPlanOpenApiPaths = {
  "/api/v1/workspaces/{workspaceId}/report-plans": {
    ...
    post: operation(
      "createReportPlan",
      "Save a private draft or publish a member report, log or plan",
  ...
  "/api/v1/report-plans/{id}": {
    get: operation(
      "getReportPlan",
      "Read an accessible report, log or plan",
      [pathParameter("id")],
      z.toJSONSchema(reportPlanSchema),
    ),
    patch: operation(
      "updateReportPlan",
      "Update your own report, log or plan",
    ...
    delete: operation(
      "archiveReportPlan",
      "Archive your own report, log or plan without deleting its history",
      [pathParameter("id"), ...writeHeaders, versionHeader],
      z.toJSONSchema(reportPlanSchema),
    ),
  },

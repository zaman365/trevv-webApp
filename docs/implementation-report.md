...
The repository also contains a versioned Hono API with explicit demo/live adapters, Better Auth composition, tenant-scoped PostgreSQL/Drizzle repositories, a typed cross-platform client, a centralized permission policy, worker/job boundaries, an Expo companion foundation, and a Tauri desktop foundation. These are implementation foundations, not an end-to-end deployed production system.
...
- 60 Playwright critical-path and visual-regression checks across desktop and mobile viewports

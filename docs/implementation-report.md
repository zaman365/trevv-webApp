...
- dependency audit reporting five advisories: three moderate build-time findings, plus two high `image-size` findings narrowly allowlisted to the Expo/Metro toolchain under `apps/mobile` because no patched release exists; any other high or critical finding fails the gate
